...
export interface ClipEffectsValues {
  ...
  sharpen?: number;
  /** Motion-Blur Preset für "240Hz look". */
  motionBlur?: 'off' | 'low' | 'medium' | 'high';
  /** Phase C6 (2026-05-19): DaVinci-style Color-Wheels (Lift/Gamma/Gain × R/G/B). */
  colorWheels?: {
 ...
 *            minterpolate genutzt. Default 30.
 */
...
// !!! WORKER-KOPIE !!! Identisch mit `packages/shared/src/ffmpegArgs.ts` —
// bei Updates BEIDE Files syncen (Worker hat keine @fiano/shared dependency,
// standalone Docker deploy auf Cloud Run).
// TikTokLayout inline definiert (statt import from './types').
export type TikTokLayout = 'stacked' | 'split' | 'full';
