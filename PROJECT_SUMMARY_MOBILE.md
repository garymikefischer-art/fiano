...
| ~~A3.11.b~~ | ~~**AI-Highlight Multi-Tab integration fixes**~~ ✅ | ~~30m~~ | **Done 2026-05-17** — (1) clip-thumb-useEffect nutzt jetzt `clip.sourceIdx` (statt one-to-one i==srcIdx) → AI-Highlights bekommen Thumbnails in TikTok+Builder-Selector. (2) BuilderTab `previewSources`-Resolution + Export `builderItemPlan`-Construction nutzen `clip.sourceIdx ?? clipIndex` → AI-Highlights spielen in Preview ab + Export geht durch. (3) useEffect-Dep `[project.id, clips.length]` triggert thumb-extract auch wenn neue Highlight-Clips dazukommen. |
