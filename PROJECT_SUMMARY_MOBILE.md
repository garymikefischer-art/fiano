...
> 🎉 **APP IST LIVE IM PLAY STORE (2026-06-10).** Build 18 (`v0.0.5, vCode 18, runtime 0.0.5`, commit `574eb4f`) Google-freigegeben, 16-KB-Block gelöst, öffentlich verfügbar. Backup-Tag `build18-live-20260610`. AAB-Backup `~/Downloads/fisora-build18-v0.0.5-vc18.aab`. Desktop v0.2.3 (Mac+Win) GitHub-Release live.
>
> 🔴 **ERSTE PHASE IM NEUEN CHAT — 2 Teile (in dieser Reihenfolge):**
> **(1) SECURITY-UPDATE (Denial-of-Wallet, Server-seitig — kein App-Update/Store-Review nötig):** Die 4 kritischen + 2 hohen Cloud-Kosten-Findings aus **`SECURITY_AUDIT_2026-06-10.md`** umsetzen. Reihenfolge nach Hebel: **K-3** Per-User-Concurrency-Limit (1–2) in der Quota-RPC (Migration 011) → **K-1** Rate-Limit auf zentralen Store (Postgres-RPC statt In-Memory) → **K-2** Email-Verify als Render-Pflicht (`auth.ts`) + Trial-Gating + Per-IP-Tageslimit → **K-4** Spec-Caps (MAX_DURATION↓, motionBlur/fps an Plan, teure Kombis=2 Einheiten) → **H-1** `max-instances 10→2` als echtes €-Limit + Monitoring-Alert → **H-2** Abo-Gate auf `/v1/upload-url`. Alles = `supabase db push` + `gcloud run deploy` (Worker-Befehl in §3). **WICHTIG: App ist öffentlich → jeder kann Accounts anlegen → das ist jetzt scharf.**
> **(2) WEBSEITE FERTIGSTELLEN:** In `claude-webseite-neu/` die Download-Buttons verknüpfen: **Play-Store-Badge/Link** (App jetzt live) + **Desktop v0.2.3** (GitHub-Release). Claude baut den Diff, User lädt per world4you/FTP hoch (kein git).
>
> 🟡 **Danach:** 2 ausstehende DPA-Mail-Antworten tracken (§5) · Polish-Phasen (§5).
