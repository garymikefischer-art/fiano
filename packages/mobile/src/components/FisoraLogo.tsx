/**
 * Fisora Logo (Phase Rebrand 2026-05-26).
 * - 'wordmark': Pfeil-Symbol links + "fisora"-Schriftzug rechts (Buchstaben weiß/theme.text.primary)
 * - 'mark':     nur Pfeil-Symbol (= App-Icon-Bestandteil)
 *
 * SVG-Daten aus assets/fisora-logo.svg (Original-Source). Wordmark-viewBox
 * 941.25 × 182.68 (ratio ~5.15) → bei `height={72}` ergibt das Breite ~371px,
 * sichtbar länger als das alte fiano-wordmark (ratio 1.75), aber der Buchstabe
 * mehr (6 vs 5) plus weniger Whitespace im neuen Design ist beabsichtigt.
 */
...
export function FisoraLogo({ variant = 'wordmark', height = 56, letterColor }: Props) {
  // Default letter-color folgt theme. Explicit letterColor-Prop overridet.
  const colors = useColors();
  const resolvedLetterColor = letterColor ?? colors.text.primary;

  if (variant === 'mark') {
    // Pfeil-only — nutzt das tight bbox des Pfeil-Symbols aus dem SVG.
    const ratio = 245.78 / 182.68;
    return (
      <Svg width={height * ratio} height={height} viewBox="0 0 245.78 182.68">
        <Polygon
          fill="#ff1039"
  ...
  // wordmark — Pfeil-Symbol + "fisora"-Schriftzug.
  // Phase Rebrand Iter 2 (2026-05-26): vorher viewBox 941.25×182.68 (ratio 5.15)
  // ergab bei height=72 ein ~371px breites Logo — visuell viel größer als das
  // alte fiano-Wordmark (das ratio 1.75 hatte und damit nur ~126px breit war).
  // Fix: viewBox um vertikales Padding erweitern auf Höhe 538, damit ratio
  // wieder ~1.75 = alte Größe. Content wird zentriert positioniert.
  const ratio = 941.25 / 538;
  return (
    <Svg width={height * ratio} height={height} viewBox="0 -177.66 941.25 538">
