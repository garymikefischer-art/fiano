...
export interface TikTokExportOpts {
  ...
  intro?: {
     ...
     *  Wird VOR overlay angewendet → Source-Video schaut durch transparente
     *  Areas. Default color=#00ff00, similarity=0.18, blend=0.08.
     *  Im 'before' Mode ignoriert (kein Source under intro). */
...
export function buildTikTokExportArgs(
  ...
  if (opts.intro && introInputIdx >= 0 && introMode === 'overlay') {
    ...
    // gesetzt. Wird zwischen scale und overlay eingehängt — green pixels werden
    // transparent → Source-Video schaut durch.
  ...
  if (opts.watermark && watermarkInputIdx >= 0) {
    ...
    const padding = 20;
    // format=rgba damit Alpha-channel respected; colorchannelmixer=aa multipliziert
    // alpha mit opacity (PNG mit alpha → reduziert; JPG/PNG ohne alpha → globaler fade).
    filters.push(
